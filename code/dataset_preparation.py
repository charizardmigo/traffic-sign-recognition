...
def split_yolo_dataset(dataset_path, output_path, train_ratio=0.7, val_ratio=0.2, test_ratio=0.1, seed=42):
    ...
    # Finds images and labels directories
    images_dir = None
    labels_dir = None
    ...
    # Filters images that have corresponding label files
    valid_pairs = []
    missing_labels = []
    ...
    # Copies data.yaml if it exists and creates a new one for the split dataset
    data_yaml_path = dataset_path / 'data.yaml'
    if data_yaml_path.exists():
        print(f"\nFound data.yaml, creating new configuration...")
        
        # Loads original data.yaml
        with open(data_yaml_path, 'r') as f:
            original_data = yaml.safe_load(f)
        
        # Creates new data.yaml for split dataset
        new_data = {
    ...
    # Creates class distribution analysis
...
def convert_to_coco_format(yolo_dataset_path, output_path):
    ...
    for split in ['train', 'val', 'test']:
        ...
        # Creates COCO annotation structure
        coco_data = {
            "info": {
        ...
        for image_id, image_file in enumerate(image_files, 1):
            ...
            # Reads corresponding YOLO annotation
            label_file = labels_dir / f"{image_file.stem}.txt"
...
def main():
    ...
    try:
        ...
        # Optionally creates COCO format
        if args.create_coco:
